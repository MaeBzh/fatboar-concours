...
export class EmailingListRepositoryMock extends Repository<EmailingList> {
  ...
  async findOneOrFail(search: any) {
    let list: EmailingList = this.database.emailingLists.find(
      (list: EmailingList) => list.id == search
    );
    if (list) {
      return list;
    }
  ...
  async save(createDto: any): Promise<any> {
    const list = {
      id:
        this.database.emailingLists[this.database.emailingLists.length - 1].id +
        1,
      ...createDto,
      winningTickets: [],
    };
    ...
    if (
      !this.database.emailingLists.some((list) => list.name === createDto.name)
    ) {
  ...
  async update(search: any, updateDto: any): Promise<any> {
    ...
    if (this.database.emailingLists.some((list) => list.id === search)) {
      listToUpdate = this.database.emailingLists.findIndex(
        (list) => list.id == search
      );
