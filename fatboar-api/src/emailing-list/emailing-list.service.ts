...
import { DeleteResult, EntityManager, FindOneOptions, Repository, UpdateResult, FindManyOptions } from 'typeorm';
...
export class EmailingListService {
  ...
  async create(createEmailingListDto: CreateEmailingListDto, manager?: EntityManager): Promise<EmailingList> {
    const users: User[] = await Promise.all(createEmailingListDto.userIds.map(async id => {      
  ...
  update(id: number, updateEmailingListDto: UpdateEmailingListDto, manager?: EntityManager): Promise<UpdateResult> {
    const repo = manager?.getRepository(EmailingList) || this.emailingRepo;
