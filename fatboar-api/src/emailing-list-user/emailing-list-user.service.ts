...
import { DeleteResult, EntityManager, FindOneOptions, Repository, SelectQueryBuilder } from 'typeorm';
...
export class EmailingListUserService {
  ...
  async create(emailingListDto: EmailingListUserDto, manager?: EntityManager): Promise<EmailingListUser> {
    const user: User = await this.usersService.findOne(emailingListDto.userId);
    const emailingList: EmailingList = await this.emailingListsService.findOne(emailingListDto.emailingListId);

    const emailingListUser = new EmailingListUser();
    emailingListUser.user = user;
    emailingListUser.emailingList = emailingList;

    const repo = manager?.getRepository(EmailingListUser) || this.emailingListUserRepo;
